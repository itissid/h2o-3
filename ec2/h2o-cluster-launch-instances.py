...
os.environ['AWS_SSH_PRIVATE_KEY_FILE'] = '~/.ssh/sid-aws-key.pem'
...
amiId = 'ami-72e9330f' # Copied over from my AWS account
...
print ('Using boto version', boto.__version__)
if True:
    botoVersionArr = boto.__version__.split(".")
    print(botoVersionArr)
    if (int(botoVersionArr[0]) != 1):
        botoVersionMismatch
    if (int(botoVersionArr[1]) <= 6):
        botoVersionMismatch
